...
;***********************************************
GPIOC EQU 0x4005A000		;puerto C dir base
...
SYSCT_RCGCGPIO_R2	EQU 0x00000004		;valor para activar el puerto C
SYSCTL_PRGPIO		EQU	0x400FEA08		;dirección para verificar puerto listo
SYSCTL_PRGPIO_R2	EQU 0x00000004
...
;-----------------------------------------------------------------------------	
    ...
    LDR R0, [R1]                    ; R0 = [R1] (lee valor)
    ORR R0, R0, #0x04               ; R0 = (Configuro pin 2 del puerto C como salida)
    STR R0, [R1]	
;-------------------------------------------------------------------------------
    ...
    LDR R0, [R1]                    ; R0 = [R1] (lee valor)
    BIC R0, R0, #0x04               ; R0 = R0&~0x01 (dehabilitafunción alternativa de PF0)
    STR R0, [R1] 	
;-------------------------------------------------------------------------------	
    ; set digital enable register
    LDR R1, =GPIO_PORTC_DEN       ; R1 = GPIO_PORTF_DEN_R (pointer)
    LDR R0, [R1]                    ; R0 = [R1] (value)
    ORR R0, R0, #0x04               ; R0 = R0|0x01 (enable digital I/O on PF0)
    STR R0, [R1]    				; [R1] = R0
;------------------------------------------------------------------------------------
